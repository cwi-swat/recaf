...
public class TestAsyncComplex {
  ...
  AsyncExtension<Integer> $alg = new AsyncExtension<Integer>();
  return (CompletableFuture<Integer>)$alg.Method($alg.Seq($alg.If($alg.Exp(() -> { return 1 < 2; }), $alg.Await($alg.Exp(() -> { return op(); }), (Integer x) -> { return $alg.Await($alg.Exp(() -> { return op(); }), (Integer y) -> { return $alg.Return($alg.Exp(() -> { return x + y; })); }); })), $alg.Seq($alg.ExpStat($alg.Exp(() -> { return pause(5000); })), $alg.Return($alg.Exp(() -> { return 41; })))));
}
