...
public class TestMaybe {
  ...
  return (Optional<Integer>)$alg.Method($alg.<Integer>Decl($alg.Exp(() -> { return 3; }), x -> {return $alg.Seq($alg.ExpStat($alg.Exp(() -> { return println("maybe"); })), $alg.If($alg.Exp(() -> { return 7 > 5; }), $alg.Return($alg.Exp(() -> { return 42; }))));}));
}
