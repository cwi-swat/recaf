...
tuple[LocalVarDec,Names] fp2decl((FormalParam)`<Type t> <Id x>`, Names names) {
  Id y = gensym(x, names.renaming<1>);
  t2 = boxed(t);
  ld = (LocalVarDec)`Ref <Id y> = new Ref(<Id x>)`;
...
Expr stm2alg((Stm)`if (<Expr c>) <Stm s>`, Id alg, Names names) 
  = (Expr)`<Id alg>.If(() -\> { return (Boolean) <Expr ecps>;}, <Expr scps>)`
  when 
    Expr ecps := expr2alg(c, alg, names),
    Expr scps := stm2alg(s, alg, names);

Expr stm2alg((Stm)`if (<Expr c>) <Stm s1> else <Stm s2>`, Id alg, Names names) 
  = (Expr)`<Id alg>.If(() -\> { return (Boolean) <Expr ecps>;}, <Expr s1cps>, <Expr s2cps>)`
  when 
    Expr ecps := expr2alg(c, alg, names),
...
Expr unwrapRefs(Expr e, Names names) {
  ...
  // ugh this is ugly.
  return visit (e) {
   case (LHS) `<Id x>` => (LHS) `<Id x>`
     when x in names.refs, x notin names.renaming
     
   case (Expr) `<Id x>` => (Expr)`<Id x>` 
     when x in names.refs, x notin names.renaming

   case (AmbName) `<Id x>` => (AmbName)`<Id x>` 
     when x in names.refs, x notin names.renaming
   ...
   case (Expr) `<Id x>++` => (Expr) `<Id y>++`
	...
	case (Expr) `<Id alg>.PostIncr(<Expr e>, <Id x>.value)` =>  (Expr) `<Id alg>.PostIncr(<Expr e>, <Id x>)`
  }    
}
...
Expr desugarExpr((Expr) `<IntLiteral n>`, Id alg, Names names)  
   = (Expr)`<Id alg>.Lit(<IntLiteral n>)`;

Expr desugarExpr((Expr) `<Id id>`, Id alg, Names names)  
   = (Expr)`<Id alg>.Var(<StringLiteral s>, <Id id>)`
   when 
     StringLiteral s := [StringLiteral]"\"<id>\"";   
     
Expr desugarExpr((Expr) `<Id id>++`, Id alg, Names names)  
   = (Expr)`<Id alg>.PostIncr(<StringLiteral s>, <Id id>)`
   when 
   	StringLiteral s := [StringLiteral]"\"<id>\"";   
     
Expr desugarExpr((Expr) `<Expr e>++`, Id alg, Names names)  
   = (Expr)`<Id alg>.PostIncr(<Expr desugaredE>)`
   when 
   	Expr desugaredE := desugarExpr(e, alg, names);
   	
Expr desugarExpr((Expr) `<Expr e1> + <Expr e2>`, Id alg, Names names)  
   = (Expr)`<Id alg>.Add(<Expr desugaredE1>, <Expr desugaredE2>)`
   when 
   	Expr desugaredE1 := desugarExpr(e1, alg, names),
   	Expr desugaredE2 := desugarExpr(e2, alg, names);   	
   	
Expr desugarExpr((Expr) `<Expr e1> \< <Expr e2>`, Id alg, Names names)  
   = (Expr)`<Id alg>.LessThan(<Expr desugaredE1>, <Expr desugaredE2>)`
   when 
   	Expr desugaredE1 := desugarExpr(e1, alg, names),
   	Expr desugaredE2 := desugarExpr(e2, alg, names);

Expr desugarExpr((Expr) `++<Expr e>`, Id alg, Names names)  
   = (Expr)`<Id alg>.PreIncr(<Expr desugaredE>)`
   when 
   	Expr desugaredE := desugarExpr(e, alg, names);
   	
Expr desugarExpr(Expr e, Id alg, Names names)
  = e;
